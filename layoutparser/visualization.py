...
def draw_box(
    ...
    for idx, ele in enumerate(layout):
        ...
        # print("12313", ele.block)
        # # angle =  eval( ele.block )
        # print(ele.block.x_1)
        # print("aaa", ele.score)
        outline_color = (
        ...
        if show_element_id or show_element_type:
            ...
            # )
            detect_res["id"] = str(ele_id)
            detect_res["type"] = str(ele.type)
            detect_res["x_1"] = (ele.block.x_1)
            detect_res["y_1"] = (ele.block.y_1)
            detect_res["x_2"] = (ele.block.x_2)
            detect_res["y_2"] = (ele.block.y_2)

            # detect_res["Rectangle"] = ele.block
            detect_res["score"] = (ele.score)
            # roi_img = canvas.crop((ele.block.x_1, ele.block.y_1, ele.block.x_2, ele.block.y_2))
            # roi_img = canvas.crop((ele.block.x_1, ele.block.y_1, ele.block.x_2, ele.block.y_2))
            # roi_img.show()
            # Draw the ids
            # draw.text(
            ...
            # )
            # print("text", text)
            print("all", start_x, start_y, start_x + text_w, start_y + text_h)
            visualization_res.append(detect_res)
